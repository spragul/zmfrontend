...
import { Menu, MenuItem, Button, Box, Paper, makeStyles, Grid, IconButton, Typography, Modal, AppBar, Toolbar, Divider } from "@material-ui/core";
    ...
    if (valueId) {
      axios.get(`${mainurl}/authenticated/${valueId}`).then((response) => {
        if (response.data.messages === "true") {
          setImgUrl(response.data.data.picurL);
          setIsLoggedIn(true);
          ...
          <Box textAlign="center">
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <Button className={classes.signInButtons} color="secondary" variant="contained" startIcon={<GTranslateIcon />} onClick={() => openloginpage()}>
                  Login
                </Button>
              </Grid>
              <Grid item xs={12} md={6}>
                <Button className={classes.signInButtons} color="secondary" variant="contained" startIcon={<GTranslateIcon />} onClick={() => signuppage()}>
                  Signin
                </Button>
              </Grid>
            </Grid>
          ...
          <Grid item xs={12} md={6}>
            <Box style={windowWidth >= 980 ? { paddingTop: "5vh", marginLeft: "5vw", marginRight: "3vw" } : { paddingTop: "1vh", marginLeft: "1vw", marginRight: "1vw" }} textAlign="left">
              <Box style={{ paddingTop: "2vh", paddingLeft: "3vw" }}>
                <Typography variant="h2">Welcome To Zoom Metting Clone</Typography>
